...
%% USER INPUT
    ...
    %definition of desired banking values for sim
    banking_min=0; %%Banking angle in deg (negative values relate to left hand corner banking)
    banking_max=-25; %Banking angle in deg (negative values relate to left hand corner banking)
    banking_steps=6; %Banking angle in deg (negative values relate to left hand corner banking)
...
%% main YMD calculation
    ...
    banking_range=linspace(banking_min, banking_max, banking_steps); %linear interpolation of banking range
    
    %Initialize waitbar
    f = waitbar(0,'Initializing...');
    overall_length=max(size(ax_range))*max(size(v_range))*max(size(delta_range))*max(size(beta_range)*max(size(banking_range)));
    z = 0;
    ...
    inti_ba=1; %internal Variable for banking loop
    ...
    ay_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    dphi_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    ay_std_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    ym_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_fl_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_fr_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_rl_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_rr_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_f_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_r_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sa_delta_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    stability_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    sideslip_damping_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    control_moment_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    control_force_table=zeros(beta_steps, delta_steps, v_steps, ax_steps, banking_steps);
    kpi=zeros(2, 19, v_steps, ax_steps, banking_steps);

    %create one plot for each velocity in v_range and ax in ax_range and banking in banking range and save to directory

    %loop for banking
    for inti_ba=1:max(size(banking_range))
        banking=banking_range(inti_ba);
        %loop for velocity
            ...
            for inti_a=1:max(size(ax_range))
                ...
                for inti_d=1:max(size(delta_range))
                    ...
                    for inti_b=1:max(size(beta_range))
                        ...
                        ym_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=ym_mean;
                        dphi_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=dphi_mean;
                        ay_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=ay_mean;
                        ay_std_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=ay_std;
                        sa_fl_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=sa_fl_mean;
                        sa_fr_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=sa_fr_mean;
                        sa_rl_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=sa_rl_mean;
                        sa_rr_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=sa_rr_mean;
                        sa_f_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=(sa_fl_mean+sa_fr_mean)/2;
                        sa_r_table(inti_b,inti_d,inti_v,inti_a, inti_ba)=(sa_rl_mean+sa_rr_mean)/2;
                ...
                %around
                if sim_model==2 || sim_model==3
                    ym_table(:,:,inti_v,inti_a, inti_ba)=ym_table(:,:,inti_v,inti_a, inti_ba).*-1;
                end

                %create and save YMD plots to specified directory
                plot_YMD
                baseFileName=['YMD_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps_banking' num2str(abs(banking_range(inti_ba))) ];
                ...
                baseFileName=['stability_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps, banking' num2str(abs(banking_range(inti_ba))), 'deg' ];
                fullFileName = fullfile(path2outputs, baseFileName);
                saveas(figure(1),fullFileName)

                baseFileName=['sideslip_damping_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps, banking' num2str(abs(banking_range(inti_ba))), 'deg' ];
                fullFileName = fullfile(path2outputs, baseFileName);
                saveas(figure(2),fullFileName)

                baseFileName=['control_moment_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps, banking' num2str(abs(banking_range(inti_ba))), 'deg'  ];
                fullFileName = fullfile(path2outputs, baseFileName);
                saveas(figure(3),fullFileName)

                baseFileName=['control_force_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps, banking' num2str(abs(banking_range(inti_ba))), 'deg'  ];
                fullFileName = fullfile(path2outputs, baseFileName);
               ...
               %create Slip angle delta plot
                plot_SA_delta
                baseFileName=['SA_delta_ax' num2str(ax_range(inti_a)) 'mps2_v' num2str(v_range(inti_v)) 'mps, banking' num2str(abs(banking_range(inti_ba))), 'deg'  ];
                fullFileName = fullfile(path2outputs, baseFileName);
                saveas(figure(1),fullFileName)
                close all
        ...
        for inti_a=1:max(size(ax_range))
            plot_3d_v_YMD
            baseFileName=['3D_velocity_YMD_ax' num2str(ax_range(inti_a)) 'mps2_banking' num2str(abs(banking_range(inti_ba)))];
            fullFileName = fullfile(path2outputs, baseFileName);
            saveas(figure(1),fullFileName)
            close all
        end
    ...
    fullFileName = fullfile(path2outputs, baseFileName);
    save(fullFileName,'ay_table','ym_table','tv_factor','brake_balance',...
        'sim_model','delta_range','beta_range','v_range','ax_range','banking_range','kpi','kpi_descr',...
